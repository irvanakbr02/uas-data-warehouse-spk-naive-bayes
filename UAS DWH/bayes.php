...
class Bayes
{
  ...
  function probNilai($nilai,$status)
  {
    $data = file_get_contents($this->pegawai);
    $hasil = json_decode($data,true);
    ...
    $t = 0;
    foreach ($hasil as $hasil) {
      if($hasil['nilaiun'] == $nilai && $hasil['status'] == $status){
        $t += 1;
      }else if($hasil['nilaiun'] == $nilai && $hasil['status'] == $status){
        $t +=1;
      }
    }
    return $t;
  ...
  function probKesehatanmata($kesehatanmata,$status)
  {
    $data = file_get_contents($this->pegawai);
    $hasil = json_decode($data,true);
    ...
    $t = 0;
    foreach ($hasil as $hasil) {
      if($hasil['kesehatanmata'] == $kesehatanmata && $hasil['status'] == $status){
        $t += 1;
      }else if($hasil['kesehatanmata'] == $kesehatanmata && $hasil['status'] == $status){
        $t +=1;
      }
    }
    return $t;
  ...
  function probKesehatanjasmani($kesehatanjasmani,$status)
  {
    $data = file_get_contents($this->pegawai);
    $hasil = json_decode($data,true);
    ...
    $t = 0;
    foreach ($hasil as $hasil) {
      if($hasil['kesehatanjasmani'] == $kesehatanjasmani && $hasil['status'] == $status){
        $t += 1;
      }else if($hasil['kesehatanjasmani'] == $kesehatanjasmani && $hasil['status'] == $status){
        $t +=1;
      }
    }
    return $t;
